...
- Web review round 1 (1 Critical: invented Foundry test names on /judge; 6 Important) — the fix was IN FLIGHT when context was saved: `web/` has uncommitted modified files from the fixer. First action next session: inspect `git status`, run `cd web && bun test && bun run build`, read `.superpowers/sdd/PLAN/task-4-report.md` "Fix round 1", commit, then run the scoped re-review against `.superpowers/sdd/PLAN/task-4-review.md`.
...
1. Land web fix round 1 (commit, scoped re-review, ledger), then deploy to Vercel and fill `{{LIVE_URL}}` in README/docs/SUBMISSION; retake screenshots from the live URL; push.
