...
Twelve of these attacks are also fired at the deployed contracts, not only at mocks: `bun run worker/src/cli.ts attack` sends each as a read-only `eth_call` to CC3 testnet with real proofs, and every one is refused with its named revert (recorded in `evidence/attacks.json`, re-run every six hours by Vercel Cron through `/api/cron/attacks`, and live on `/judge`). Beyond the tests, the suites are measured: 93.6% line and 93.5% branch coverage, a 95.8% mutation score whose 8 survivors are equivalent mutants, and a triaged Slither run, all in [`docs/MEASUREMENTS.md`](MEASUREMENTS.md).
...
**A bought or rented World ID is a real borrower.** The contract cannot tell a purchased identity from its owner. Exposure per identity is bounded by deployed parameters: a fresh line is 25 hUSD; the `CreditHistory` boost is 25% of proven Aave V3 repayments capped at 500 hUSD; an Ethereum wallet links to one human only (`WalletAlreadyLinked`); and a repayment counts only `MIN_GAP_BLOCKS` = 7,200 source blocks after its borrow. The full boost therefore needs 2,000 USD of real, repaid Aave borrowing per identity, and the worst case is a 525 hUSD line lost once per identity. Aggregate lending stays under `exposureCap()` from `0x0FD4`. A boost that vests over on-time Creditcoin terms would shrink this further and is not built.

**A stolen World ID credential takes the line with it.** `HumanRegistry.register` re-binds a nullifier to whichever wallet presents a valid proof, with no cooldown and no check on the line's balance. An attacker who controls someone's World App identity can re-bind, draw the earned limit, and leave `LineFrozen` on the victim's nullifier. Credential custody is World's security boundary. A production version should delay re-binds while a line owes money, so the original wallet has a window to repay or object.
