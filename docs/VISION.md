...
Live: {{LIVE_URL}} · Repo: {{REPO_URL}} · Demo: {{VIDEO_URL}}
...
`HumanGate` ({{ADDRESS_HUMAN_GATE}}) is the twenty-line example that shows a third party how to gate anything on personhood. `hUSD` ({{ADDRESS_HUSD}}) is the test stablecoin the pool lends, six decimals so amounts read like dollars.
